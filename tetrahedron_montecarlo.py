...
def calculate_r_tetra_volume(total_points, lower, upper):
    ...
    while iterations < total_points:
        ...
        if (
            (x**2 + y**2 + (z - root_2) ** 2 <= 4)
            and (x**2 + (y - root_2) ** 2 + z**2 <= 4)
            and ((x - root_2) ** 2 + y**2 + z**2 <= 4)
            and ((x - root_2) ** 2 + (y - root_2) ** 2 + (z - root_2) ** 2 <= 4)
        ):  # eqns of spheres as found in prevoius class exercises
            inside_count += 1
...
if __name__ == "__main__":
    ...
    lower, upper = (
        -1,
        2,
    )  # Bounds for the bounding box    //-1,2 give 100 percent accuracy , -0.5 , 1.6 gives about 98 percent accuarcy
